...
/**
 * This interface provides parsing support from Qi4j queries to SQL queries. The idea is that this interface produces SQL which can be used when
...
{
   /**
    * This method will be called when parsing needs to be done from Qi4j query to SQL query. This method is supposed to return one single SQL query,
    * which can be used with {@link PreparedStatement}.
    ...
    * @param values Values to put into {@link PreparedStatement}, in the order of the list. This List is created from the outside and this method is supposed to fill it with necessary values, if any.
    * @param valueSqlTypes The SQL types of the objects in {@code values}. Each SQL type at index {@code x} is interpreted as type of value at index {@code x} of the {@code values} -list.
