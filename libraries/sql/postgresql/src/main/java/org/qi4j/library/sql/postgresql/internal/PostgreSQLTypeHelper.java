...
/**
 * A little helper interface to convert java types to SQL types when inserting objects to prepared statements. It will first preprocess the object and possibly convert it to something else (like Character to Integer).
 * Then it will use the {@link SQLIndexingState#javaTypes2SQLTypes()} mapping to determine which SQL datatype to use when inserting the object to prepared statement.
 * 
   ...
   public class SQLTypeHelperMixin implements PostgreSQLTypeHelper
   {
      ...
      public Object processJavaPrimitiveBeforeUsingInStatement(Object primitive)
      {
         if (primitive != null)
         {
            ...
            {
               primitive = Character.codePointAt(new char[] { (char) (Character) primitive }, 0);
            }
            ...
            {
               primitive = (short) (byte) (Byte) primitive;
            }
            else if (primitive instanceof Short)
            {
               primitive = (int) (short) (Short) primitive;
            }
         }
         return primitive;
      }
