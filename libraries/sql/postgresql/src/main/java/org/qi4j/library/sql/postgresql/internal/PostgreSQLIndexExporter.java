...
import org.qi4j.library.sql.common.GenericDatabaseExplorer;
import org.qi4j.library.sql.common.GenericDatabaseExplorer.ColumnInfo;
import org.qi4j.library.sql.common.GenericDatabaseExplorer.DatabaseProcessor;
import org.qi4j.library.sql.common.GenericDatabaseExplorer.ForeignKeyInfo;
      ...
      try
      {
          GenericDatabaseExplorer.visitDatabaseTables( this._state.connection( ).get( ), null, this._state.schemaName( ).get( ), null, new DatabaseProcessor( )
        {
            ...
            public void beginProcessColumnInfo(String schemaName, String tableName, ColumnInfo colInfo, ForeignKeyInfo fkInfo)
            {
               ...
               out.write("<column name=\"" + colInfo.getName( ) + "\" colType=\"" + colInfo.getTypeName( ) + "\" colSize=\"" + colInfo.getSize( ) + "\" colScale=\"" + //
                     colInfo.getScale( ) + "\" nullable=\"" + colInfo.getNullable( ) + "\" default=\"" + defaultValue + "\" " //
                     );
               if (fkInfo != null)
               {
                  out.write("refSchemaName=\"" + fkInfo.getPkSchemaName( ) + "\" refTableName=\"" + fkInfo.getPkTableName( ) + "\" refPKColumnName=\"" + fkInfo.getPkTablePKColumnName( ) + //
                        "\" onUpdate=\"" + fkInfo.getOnUpdateAction( ) + "\" onDelete=\"" + fkInfo.getOnDeleteAction( ) + "\" deferrability=\"" + fkInfo.getDeferrability( ) + "\" " //
                        );
               }
      ...
      try
      {
          GenericDatabaseExplorer.visitDatabaseTables( this._state.connection( ).get( ), null, this._state.schemaName( ).get( ), null, new DatabaseProcessor()
         {
            ...
            public void beginProcessColumnInfo(String schemaName, String tableName, ColumnInfo colInfo, ForeignKeyInfo fkInfo)
            {
               ...
               if (fkInfo != null)
               {
                  out.print(" -> " + fkInfo.getPkSchemaName( ) + "." + fkInfo.getPkTableName( ) + "(" + fkInfo.getPkTablePKColumnName( ) + ")[ON UPDATE " + fkInfo.getOnUpdateAction( ) + ", ON DELETE " + fkInfo.getOnDeleteAction( ) + ", " + fkInfo.getDeferrability( ) + "]");
               }
               out.print("\n");
            }
