...
{
    public EntityListViewPanel( @Uses String id, @Uses IModel model, @Structure final ObjectBuilderFactory factory )
    {
        super( id, model );
        Query<EntityComposite> query = (Query<EntityComposite>) model.getObject();
        createHeaders( factory, query );
        DataView rows = createRows( factory, query );
        add( new PagingNavigator( "navigator", rows ) );
    }

    private void createHeaders( ObjectBuilderFactory factory, Query<EntityComposite> query )
    {
    ...
    private DataView createRows( final ObjectBuilderFactory factory, final Query<EntityComposite> query )
    {
        DataView rows = new DataView( "rows", new QueryDataProvider( query ) )
        {
            protected void populateItem( final Item item )
            {
                EntityComposite entity = (EntityComposite) item.getModelObject();
                ObjectBuilder<EntityDataProvider> builder = factory.newObjectBuilder( EntityDataProvider.class );
                builder.use( entity );
                DataView columns = new DataView( "columns", builder.newInstance() )
                {
                    protected void populateItem( Item item )
                    {
                        Label label = new Label( "column", item.getModelObject().toString() );
                        item.add( label );
                    }
                };
                item.add( columns );
                item.add( new AttributeModifier( "class", true, new AbstractReadOnlyModel()
                {
                    ...
                    {
                        return ( item.getIndex() % 2 == 1 ) ? "even-row" : "odd-row";
                    }
                } ) );
            }
