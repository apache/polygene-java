...
public class ConstraintViolationInterceptor extends AbstractInterceptor {
    ...
    private void overrideActionValues(ActionInvocation invocation, ValueStack stack, final HashMap<String, Object> propertyOverrides) {
    ...
    private void addConstraintViolationFieldErrors(ValueStack stack, ValidationAware va, String fieldName, FieldConstraintViolations violations) {
        for (ConstraintViolation constraintViolation : violations.constraintViolations()) {
            String message = message(violations.target(), violations.propertyName(), constraintViolation, stack);
    ...
    private Map<String, FieldConstraintViolations> fieldConstraintViolations(ActionContext context) {
        Map<String, FieldConstraintViolations> violations = (Map<String, FieldConstraintViolations>) context.get(CONTEXT_CONSTRAINT_VIOLATIONS);
        if (violations == null) {
            return emptyMap();
        }
        return violations;
    ...
    protected String message(Composite target, String propertyName, ConstraintViolation constraintViolation, ValueStack stack) {
     ...
     */
    protected String messageKey(Composite target, String propertyName, ConstraintViolation constraintViolation) {
        return classNameInDotNotation(target.type(), withoutCompositeOrEntitySuffix)
             + "." + propertyName
             + "." + constraintKeyPart(constraintViolation)
             + ".constraint.violated";
    }
    ...
    public static class FieldConstraintViolations {
        ...
        public FieldConstraintViolations(Composite target, 
                String propertyName, 
                Object value, 
