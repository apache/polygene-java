 ...
 * Uses GigaSpaces extensions to Java Spaces (IJSpace and ExternalEntry).
 */
public class JavaSpacesEntityStoreMixin implements EntityStore, Activatable {
	private IJSpace space;

	public org.qi4j.spi.entity.EntityState getEntityState(
			org.qi4j.spi.structure.CompositeDescriptor compositeDescriptor,
			QualifiedIdentity identity) throws EntityStoreException {
		...
		try {
			ExternalEntry entry = (ExternalEntry) space.read(template, null,
					1000);
			if (entry == null) {
				throw new EntityNotFoundException("Space", identity
						.getIdentity());
			}
			Map<String, Object> properties = new HashMap<String, Object>();
			for (int indx = 0; indx < entry.m_FieldsNames.length; indx++) {
				properties.put(entry.m_FieldsNames[indx],
						entry.m_FieldsValues[indx]);
			}
			return new EntityStateInstance(0, identity, EntityStatus.LOADED,
	...
	public org.qi4j.spi.entity.EntityState newEntityState(
			org.qi4j.spi.structure.CompositeDescriptor arg0,
			QualifiedIdentity id) throws EntityStoreException {
	...
	public StateCommitter prepare(Iterable<EntityState> newStates,
			...
			Iterable<QualifiedIdentity> removedStates, Module module)
...
//		space = (IJSpace) SpaceFinder.find("jini://localhost/*/TrainingSpace?groups=dummy");
		space = (IJSpace) SpaceFinder.find("/./Qi4jSpace?groups=qi4j");
	}
