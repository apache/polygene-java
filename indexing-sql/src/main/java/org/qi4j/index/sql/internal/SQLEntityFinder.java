    ...
    implements EntityFinder
{
    ...
    public Iterable<EntityReference> findEntities( String resultType, BooleanExpression whereClause, OrderBy[] orderBySegments, final Integer firstResult, final Integer maxResults )
        throws EntityFinderException
    {
        ...
        if( maxResults == null || maxResults > 0 )
        {
            ...
            result = this.performQuery( new DoQuery<Iterable<EntityReference>>()
            {
                ...
                {
                    PreparedStatement ps = null;
                    List<EntityReference> resultList = new ArrayList<EntityReference>(
    ...
    private PreparedStatement createPS( String query, List<Object> values, List<Integer> valueSQLTypes, Integer resultSetType, Integer resultSetHoldability )
        throws SQLException
    {
        PreparedStatement ps = this.state.connection().get().prepareStatement( query, resultSetType,
                                                                                ResultSet.CONCUR_READ_ONLY,
                                                                                resultSetHoldability );
        if( values.size() != valueSQLTypes.size() )
        {
            throw new InternalError(
                "There was either too little or too much sql types for values [values=" + values.size() + ", types=" + valueSQLTypes.size() + "]." );
        }
