 ...
 * 
 * See {@link #fromProperty(QualifiedName, List, String, Type, String)},
 * {@link #fromAssociation(QualifiedName, String, Type)} and {@link #fromManyAssociation(QualifiedName, String, Type)}
...
{
    /**
     * Currently all possible types of qualified names: {@link #PROPERTY} for properties, {@link #ASSOCIATION} for
     * associations, and {@link #MANY_ASSOCIATION} for many-associations.
     */
    ...
    )
    {
        ...
        this._collectionClasses = (collectionClasses == null || collectionClasses.size() == 0 ? EMPTY_COLL_CLASSES
            : collectionClasses);
    ...
    /**
     * If qualified name represented by this interface is a property with collection as type, returns the
     * {@code amount of nested collections + 1}. That is, assuming {@code X} is not a collection, for type
     * {@code Property<Set<X>>} this returns {@code 1}, for {@code Property<Set<Set<X>>>} this returns {@code 2}, etc.
     ...
     * 
     * @return The collection depth ({@code > 0}) of qualified name, if this interface represents qualified name with
     *         collection property; {@code 0} otherwise.
     */
    ...
    /**
     * Returns the non-collection type of this qualified name. That is, for {@code Property<X>} this returns {@code X}
     * if {@code X} is not a collection type, and for {@code Property<Set<Y>>} this returns {@code Y} if {@code Y} is
     ...
     * 
     * @return The table name in database, used to store values of the qualified name this interface represents. May be
     *         {@code null} if it is not yet decided.
     */
    ...
    /**
     * Sets the previously undecided table name to some specific one. This method only works when argument is
     * {@code non-null} and current table name is {@code null}.
     * 
    ...
    /**
     * Returns whether the final (non-collection) type of this qualified name is not seen as value composite. Always
     * returns {@code false} for qualified names of type {@link QNameType#ASSOCIATION} and
     * {@link QNameType#MANY_ASSOCIATION}.
     * 
    ...
    /**
     * Returns {@link PropertyDescriptor} associated with this property, if this qualified name info represents a
     * property. Returns {@code null} otherwise.
     * 
     * @return {@link PropertyDescriptor} if this qualified name info is associated with property, {@code null}
     *         otherwise.
     */
    ...
    /**
     * Returns {@link AssociationDescriptor} associated with this association, if this qualified name info represents an
     * association. Returns {@code null} otherwise.
     * 
     * @return {@link AssociationDescriptor} if this qualified name info is associated with association, {@code null}
     *         otherwise.
     */
    ...
    /**
     * Gets the type of represented qualified name: either {@link QNameType#PROPERTY} for properties,
     * {@link QNameType#ASSOCIATION} for associations, or {@link QNameType#MANY_ASSOCIATION} for many-associations.
     * 
     * @return The type of represented qualified name: either {@link QNameType#PROPERTY}, {@link QNameType#ASSOCIATION},
     *         or {@link QNameType#MANY_ASSOCIATION}.
     */
     ...
     * @param tableName The table name where the values of all instances of propertiy with this qualified name will be
     *            stored. May be {@code null} if it is to be decided later.
     * @param propertyDescriptor {@link PropertyDescriptor} of this property.
     * @return An object representing information about property with this qualified name, and how instances of this
     *         property are stored in database.
     */
    ...
    )
    {
        ...
        while( vType instanceof ParameterizedType
            && Collection.class.isAssignableFrom( (Class<?>) ((ParameterizedType) vType).getRawType() ) )
        {
            collectionClasses.add( (Class<?>) ((ParameterizedType) vType).getRawType() );
            vType = ((ParameterizedType) vType).getActualTypeArguments()[0];
        }

        return new QNameInfo( qName, QNameType.PROPERTY, collectionClasses, tableName, vType, propertyDescriptor, null,
            null );
    }
     ...
     * @param tableName The table name where the values of all instances of association with this qualified name will be
     *            stored. May be {@code null} if it is to be decided later.
     * @param assoDescriptor {@link AssociationDescriptor} of this association.
     * @return An object representing information about association with this qualified name, and how instances of this
     *         association are stored in database.
     */
    ...
    {
        return new QNameInfo( qName, QNameType.ASSOCIATION, null, tableName, assoDescriptor.type(), null,
            assoDescriptor, null );
    }
