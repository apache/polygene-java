...
public abstract class CollectionBuilder<T extends Collection>
{
    ...
    {
        {
            put( List.class, new Applyer()
            {
                ...
                {
                    builder.buildCollection( (T) owner.list(), factory );
                }

                public void verify( UnitOfWork uow, ElementOwnerComposite elementOwner, int[] expected )
                {
                    CollectionBuilder.verify( elementOwner.list(), expected );
                }
            } );
            put( Set.class, new Applyer()
            {
                public <T extends Collection> void apply( CollectionBuilder<T> builder, ElementOwnerComposite owner, ElementFactory factory )
                {
                    builder.buildCollection( (T) owner.set(), factory );
                }

                public void verify( UnitOfWork uow, ElementOwnerComposite elementOwner, int[] expected )
                {
                    CollectionBuilder.verify( elementOwner.set(), expected );
                }
            } );
        }
    } );
    ...
    {
        return uow.newEntityBuilder( null, ElementOwnerComposite.class ).newInstance();
    }

    private static ElementOwnerComposite getElementOwner( UnitOfWork uow, String id )
    {
        return uow.getReference( id, ElementOwnerComposite.class );
    }

    private static ContainedElement newContainedElement( UnitOfWork uow, int index )
    {
        CompositeBuilder<ContainedElementComposite> builder = uow
            .newEntityBuilder( null, ContainedElementComposite.class );
        ContainedElement prototype = builder.stateOfComposite();
