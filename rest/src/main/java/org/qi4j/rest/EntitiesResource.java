...
public class EntitiesResource extends Resource
{
        ...
        throws ResourceException
    {
        try
        {
            ...
            buf.append( "<html><head><title>Entities of type " + type + "</title></head><body><h1>Entities of type " + type + "</h1><ul>" );

            for( QualifiedIdentity entity : query )
            {
                buf.append( "<li><a href=\"" + getRequest().getResourceRef().getPath() + "/" + entity.identity() + ".html\">" + entity.identity() + "</a></li>" );
            }
