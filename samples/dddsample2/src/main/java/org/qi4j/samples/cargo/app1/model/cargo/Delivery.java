            ...
            switch( lastEvent().eventType() )
            {
                ...
                case LOAD:
                    for( Leg leg : state.itinerary().get().legs() )
                    {
                        ...
                        {
                            return handlingActivityFactory.create( HandlingEvent.Type.UNLOAD, leg.unloadLocation(),
                                                                   leg.voyage() );
                        }
                    }
                    return NO_ACTIVITY;
                    ...
                    for( Iterator<Leg> it = itinerary.legs().iterator(); it.hasNext(); )
                    {
                        ...
                        if( leg.unloadLocation().equals( lastEvent().location() ) )
                        {
                            if( it.hasNext() )
                            {
                                ...
                                return handlingActivityFactory.create( HandlingEvent.Type.LOAD, nextLeg.loadLocation(),
                                                                       nextLeg.voyage() );
                            }
                            else
                            {
                ...
                case RECEIVE:
                    ...
                    return handlingActivityFactory.create( HandlingEvent.Type.LOAD, firstLeg.loadLocation(),
                                                           firstLeg.voyage() );
