function calendar_constructor()
{
    ...
    this.hideCurrent = function( event, triggerElem )
    {
        ...
        {
            if( event )
            { // if manual show/hide do not propagate normal behaviour
    ...
    function obtainInputField( calendar )
    {
        ...
        if( calendar.input.dynamicId )
        {
            ...
            if( inputToStoreValue )
            { // if found not a text input - use default one
    ...
    function createContents( elem, year, month, day )
    { // year, month, day are optional in nonfirst calls
